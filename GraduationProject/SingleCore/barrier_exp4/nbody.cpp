...
template<class float_type> class SingleTaskNBodyKernel;
...
protected:
  ...
  void submitSingleTask(sycl::buffer<particle_type>& particles, sycl::buffer<vector_type>& velocities) {
    args.device_queue.submit([&](sycl::handler& cgh) {
      //sycl::nd_range<1> execution_range{sycl::range<1>{args.problem_size}, sycl::range<1>{args.local_size}};
      ...
      const int Problem_size = args.problem_size;
      const int Local_size =args.local_size;

      cgh.single_task<SingleTaskNBodyKernel<float_type>>([=, dt = this->dt, gravitational_softening = this->gravitational_softening]() {

        for(size_t global_id=0; global_id< Problem_size; global_id++)
        {
            vector_type v = velocities_access[global_id];
            vector_type acceleration{static_cast<float_type>(0.0f)};
            particle_type my_particle = (global_id < Problem_size) ? particles_access[global_id] : particle_type{static_cast<float_type>(0.0f)};
            ...
            for(size_t offset = 0; offset < Problem_size; offset += Local_size) 
            {
              ...
              {
                scratch[local_id] = (global_id < Problem_size) ? particles_access[offset + local_id]
                                                              : particle_type{static_cast<float_type>(0.0f)};
              }
              //tid.barrier();
...
template<class float_type>
class NBodySingleTask : public NBody<float_type>
{
public:
  using typename NBody<float_type>::particle_type;
  using typename NBody<float_type>::vector_type;

  NBodySingleTask(const BenchmarkArgs& _args)
  : NBody<float_type>{_args} {}


  void run(){
    this->submitSingleTask(this->particles_buf.get(), this->velocities_buf.get());
  }

  std::string getBenchmarkName() {
    std::stringstream name;
    name << "NBody_SingleTask_";
    name << ReadableTypename<float_type>::name;
    return name.str();
  }
};
...
int main(int argc, char** argv)
{
  ...
  app.run< NBodySingleTask<float> >();
  if(app.deviceSupportsFP64())
    app.run<NBodySingleTask<double>>();
