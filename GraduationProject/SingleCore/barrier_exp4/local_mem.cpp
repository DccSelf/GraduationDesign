...
public:
  ...
  void run(std::vector<cl::sycl::event>& events) {
    events.push_back(args.device_queue.submit([&](cl::sycl::handler& cgh) {
      ...
      cgh.single_task<MicroBenchLocalMemoryKernel<DATA_TYPE, COMP_ITERS>>([=](){

          for(int global_id=0; global_id<global_size;)
          {
			int gid = global_id;
