...
class Polybench_Mvt {
  public:
	...
	void run(std::vector<cl::sycl::event>& events) {
		...
		// 	//cgh.parallel_for<Mvt1>(,[,](){});
		// 	cgh.single_task<Mvt1>([=,N_ = size](){
		// 		for(size_t i = 0; i < N_; i++) {
		// 			DATA_TYPE tmp = x1[i];
		// 			for(size_t j = 0; j < N_; j++) {
		// 				tmp += a[{i,j}] * y1[j];
		// 				//x1[j] += a[{i,j}] * y1[i];  //初始化是对称矩阵，两种结果相同，但是下面这种性能更好。
		// 			}
		// 			x1[i] = tmp;
		// 		}
		// 	});
		...
		// 	cgh.single_task<Mvt2>([=,N_ = size](){
		// 		for(size_t k = 0; k < N_; k++) {
		// 			DATA_TYPE tmp = x2[k];
		// 			for(size_t l = 0; l < N_; l++) {
		// 				tmp += a[{k,l}] * y2[l];
		// 				//x2[l] += a[{l,k}] * y2[k]; //初始化是对称矩阵，两种结果相同，但是下面这种性能更好。
		// 			}
		// 			x2[k] = tmp;
		// 		}
		// 	});
		...
		//0.5s+
		//只用一个命令组，发现性能还能提升一倍，原因：消除了barrier
		events.push_back(args.device_queue.submit([&](handler &cgh){
			auto a = a_buffer.get_access<access::mode::read>(cgh);
			auto y1 = y1_buffer.get_access<access::mode::read>(cgh);
			auto x1 = x1_buffer.get_access<access::mode::read_write>(cgh);
