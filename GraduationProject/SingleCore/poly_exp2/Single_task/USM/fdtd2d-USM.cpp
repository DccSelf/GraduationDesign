...
class Polybench_Fdtd2d {
  public:
	...
	void setup() {
		...
		int HZSIZE=size*size;
        args.device_queue.submit([&](cl::sycl::handler& h){
            auto in_d = HZ_usm;
            auto in_h = &hz[0];
            h.single_task([=,N=HZSIZE](){
				for(size_t i=0;i<N;i++)
				{
					in_d[i] = in_h[i];
				}
			...
			events.push_back(args.device_queue.submit([&](handler& cgh) {
				...
				cgh.single_task<Fdtd2d1>([=,NX=size,NY=size](){
					...
					for(size_t i = 0; i < NX; i++) {
						...
						for(size_t j = 1; j < NY; j++) {
							ex[i*NY+j] = ex[i*NY+j] - 0.5 * (hz[i*NY+j] - hz[i*NX+(j-1)]);
						}
					}

					for(size_t i = 0; i < NX; i++) {
						for(size_t j = 0; j < NY; j++) {
							hz[i*NX+j] = hz[i*NX+j] - 0.7 * (ex[i*NX+(j+1)] - ex[i*NX+j] + ey[(i+1)*NX+j] - ey[i*NX+j]);
						}
					}
