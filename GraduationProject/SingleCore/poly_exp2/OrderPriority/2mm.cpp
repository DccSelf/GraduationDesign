  ...
  public:
	...
	void run(std::vector<cl::sycl::event>& events) {
		...
		events.push_back(args.device_queue.submit([&](handler& cgh) {
			...
			auto E = E_buffer.get_access<access::mode::write>(cgh);

			cgh.parallel_for<Polybench_2mm_3>(E_buffer.get_range(), [=, size_ = size](item<2> item) {
				const auto i = item[0];
				const auto j = item[1];
			//存在E[item]的情况下无法调整行列优先，这也是在单核情况下使用basic/NDRange parallel for的缺陷
			//E[item] = 0;
