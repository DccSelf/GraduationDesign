...
// clang++ -fsycl  -O3 -I/thfs1/home/penglin/dcc/sycl-bench/include -DSYCL_BENCH_ENABLE_QUEUE_PROFILING  Scale.cpp -o Scale
// clang++ -fsycl -O3 -S -emit-llvm Scale.cpp -o Scale.ll
// clang++ -fsycl -O3 -c -emit-llvm Scale.cpp -o Scale.bc


// valgrind --tool=massif --time-unit=ms --max-snapshots=1000 --detailed-freq=1 --pages-as-heap=yes ./Scale-single_task --size=1024 --num-runs=1
...
class MicroBenchScaleKernel1;


template <typename DataT, int Dims>
class MicroBenchScaleKernel2;
...
/**
 * Microbenchmark measuring Scale bandwidth.
 */
template <typename DataT, int Dims>
class MicroBenchScale {
protected:
//   BenchmarkArgs args;
...
public:
  MicroBenchScale(const BenchmarkArgs& args):args(args),USM_size(getBufferSize<DataT, Dims>(args.problem_size).size())
  {
...
int main(int argc, char** argv) {
  ...
  app.run<MicroBenchScale<float, 1>>();
//   app.run<MicroBenchScale<float, 2>>();
//   app.run<MicroBenchScale<float, 3>>();
//   if(app.deviceSupportsFP64()) {
//     app.run<MicroBenchScale<double, 1>>();
//     app.run<MicroBenchScale<double, 2>>();
//     app.run<MicroBenchScale<double, 3>>();
//   }
