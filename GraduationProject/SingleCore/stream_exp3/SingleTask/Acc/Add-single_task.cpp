...
public:
  ...
  void run(std::vector<s::event>& events) {
    events.push_back(args.device_queue.submit([&](cl::sycl::handler& cgh) {
      ...
      // cgh.parallel_for<MicroBenchScaleKernel<DataT, Dims>>(global_size, [=](s::id<Dims> gid) {
      //   out[gid] = in1[gid] + in2[gid]; });

      //single_task
      cgh.single_task<MicroBenchAddKernel<DataT,Dims>>([=,global_size=buffer_size]{
