...
class Polybench_3DConvolution {
  public:
	...
	void setup() {
		...
		auto SIZE = size * size * size;
        A_usm = malloc_host<DATA_TYPE>(SIZE,args.device_queue);
        ...
        B_usm = malloc_host<DATA_TYPE>(SIZE,args.device_queue);
        args.device_queue.submit([&](cl::sycl::handler& h){
            auto in_d = B_usm;
            h.parallel_for(SIZE,[=](cl::sycl::id<1> i){
              in_d[i] = 0.0f;
	...
	void run(std::vector<cl::sycl::event>& events) {
		...
		events.push_back(args.device_queue.submit([&](handler& cgh) {
			...
			cgh.parallel_for<class conv3D>(range<3>(size,size,size), [=, size_=size,NK = size,NJ=size](item<3> item) {
				const auto i = item[0];
				const auto j = item[1];
				const auto k = item[2];
