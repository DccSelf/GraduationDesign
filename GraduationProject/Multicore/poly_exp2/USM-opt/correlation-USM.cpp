...
class Polybench_Correlation {
  public:
	...
	void setup() {
		...
		stddev_usm = malloc_device<DATA_TYPE>(size+1,args.device_queue); //1
		args.device_queue.submit([&](cl::sycl::handler& h){
            auto in_d = stddev_usm;
            h.parallel_for(size+1,[=](cl::sycl::id<1> i){
              in_d[i] = 0.0f;
            });
        }).wait();
	...
	void run(std::vector<cl::sycl::event>& events) {
		...
		events.push_back(args.device_queue.submit([&](handler& cgh) {
			...
			cgh.parallel_for<CorrelationStd>(range<1>(size), id<1>(1), [=, N_ = size](item<1> item) {
				...
				for(size_t i = 1; i <= N_; i++) {
					stddev[item] += (data[i*(N_+1)+j] - mean[item]) * (data[i*(N_+1)+j] - mean[item]);
				}
		...
		events.push_back(args.device_queue.submit([&](handler& cgh) {
			...
			cgh.parallel_for<CorrelationReduce>(range<2>(size, size), id<2>(1, 1), [=,N_=size](item<2> item) {
				const auto i = item[0];
				const auto j = item[1];

				data[i*N_+j] -= mean[j];
				data[i*N_+j] /= cl::sycl::sqrt(FLOAT_N);
				data[i*N_+j] /= stddev[j];
			});
		}));
