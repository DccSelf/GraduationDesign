  ...
  public:
	...
	void run(std::vector<cl::sycl::event>& events) {
		...
		events.push_back(args.device_queue.submit([&](handler& cgh) {
          ...
          cl::sycl::range num_groups{args.problem_size/args.local_size,args.problem_size/args.local_size};
          cl::sycl::range group_size{args.local_size,args.local_size};

          cgh.parallel_for_work_group<Gemm>(num_groups,group_size, [=, NK_ = size ,LOCAL = args.local_size](cl::sycl::group<2> item) {
            const auto ib = item.get_group_id(0);
            const auto jb = item.get_group_id(1);
           //C[{ib,jb}] *= BETA; // NI*NJ 乘法
            item.parallel_for_work_item([&](cl::sycl::h_item<2>it){
                auto i = ib * LOCAL + it.get_local_id(0);
                auto j = jb * LOCAL + it.get_local_id(1);
                C[{i,j}] *= BETA;
