  ...
  public:
	...
	void run(std::vector<cl::sycl::event>& events) {
		...
		events.push_back(args.device_queue.submit([&](handler& cgh) {
			...
			local_accessor<DATA_TYPE> tileA1{tile_size,cgh};
			cgh.parallel_for<Polybench_3mm_1>(nd_range<2>{{size,size},{1,size}}, [=, size_ = size](nd_item<2> item) {
				size_t i = item.get_global_id()[0];
				size_t j = item.get_global_id()[1];
				size_t m = item.get_local_id()[1];

				DATA_TYPE sum = 0;
				for(size_t kk = 0; kk < size_; kk+=tile_size) { //A[{i,k}]可重用
					tileA1[m] = A[{i,kk+m}];
					item.barrier();

					for(size_t k=0;k<tile_size;k++)
					{
						sum += tileA1[k] * B[{kk+k,j}];
					}
					item.barrier();
				}
		...
		events.push_back(args.device_queue.submit([&](handler& cgh) {
			...
			size_t tile_size = args.local_size;
			local_accessor<DATA_TYPE> tileA2{tile_size,cgh};
			cgh.parallel_for<Polybench_3mm_2>(nd_range<2>{{size,size},{1,size}}, [=, size_ = size](nd_item<2> item) {
				size_t i = item.get_global_id()[0];
				size_t j = item.get_global_id()[1];
				size_t m = item.get_local_id()[1];

				DATA_TYPE sum = 0;
				for(size_t kk = 0; kk < size_; kk+=tile_size) { //C[{i,k}]可重用
					tileA2[m] = C[{i,kk+m}];
					item.barrier();

					for(size_t k=0;k<tile_size;k++)
					{
						sum += tileA2[k] * D[{kk+k,j}];
					}
					item.barrier();
				}
		...
		events.push_back(args.device_queue.submit([&](handler& cgh) {
			...
			size_t tile_size = args.local_size;
			local_accessor<DATA_TYPE> tileA3{tile_size,cgh};
			cgh.parallel_for<Polybench_3mm_3>(nd_range<2>{{size,size},{1,size}}, [=, size_ = size](nd_item<2> item) {
				size_t i = item.get_global_id()[0];
				size_t j = item.get_global_id()[1];
				size_t m = item.get_local_id()[1];

				DATA_TYPE sum = 0;
				for(size_t kk = 0; kk < size_; kk+=tile_size) { //E[{i,k}]可重用
					tileA3[m] = E[{i,kk+m}];
					item.barrier();

					for(size_t k=0;k<tile_size;k++)
					{
						sum += tileA3[k] * F[{kk+k,j}];
					}
					item.barrier();
				}
