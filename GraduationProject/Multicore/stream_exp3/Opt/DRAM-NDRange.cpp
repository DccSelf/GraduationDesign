...
public:
  ...
  void run(std::vector<s::event>& events) {
    events.push_back(args.device_queue.submit([&](cl::sycl::handler& cgh) {
      ...
      sycl::nd_range<1> execution_range{sycl::range<1>{buffer_size.size()}, sycl::range<1>{args.local_size}};
	  //problem_size   local_size
	  cgh.parallel_for<MicroBenchDRAMKernel<DataT, Dims>>(execution_range,[=](sycl::nd_item<1> item){
