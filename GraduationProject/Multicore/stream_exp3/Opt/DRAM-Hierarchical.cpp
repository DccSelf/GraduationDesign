...
public:
  ...
  void run(std::vector<s::event>& events) {
    ...
    //   cgh.parallel_for<MicroBenchDRAMKernel<DataT, Dims>>(global_size, [=](s::id<Dims> gid) { out[gid] = in[gid]; });
        ...
        cl::sycl::range num_groups{buffer_size.size()/args.local_size};
        cl::sycl::range group_size{args.local_size};
        //只考虑一维
        cgh.parallel_for_work_group<MicroBenchDRAMKernel<DataT, Dims>>(num_groups,group_size, [=, global_size = buffer_size.size() ,LOCAL = args.local_size](cl::sycl::group<1> item) {
            const auto ib = item.get_group_id(0);
           //C[{ib,jb}] *= BETA; // NI*NJ 乘法
            item.parallel_for_work_item([&](cl::sycl::h_item<1>it){
