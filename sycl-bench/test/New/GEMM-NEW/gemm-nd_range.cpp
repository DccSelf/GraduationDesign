...
public:
  ...
  void run(std::vector<cl::sycl::event>& events) {
    ...
    events.push_back(args.device_queue.submit([&](handler& cgh) {
      ...
      cl::sycl::range global{args.problem_size,args.problem_size};
      cl::sycl::range local{args.local_size,args.local_size};

      cgh.parallel_for<Gemm>(cl::sycl::nd_range{global,local}, [=, NK_ = size](cl::sycl::nd_item<2> item) {
        ...
        for(size_t k = 0; k < NK_; k++) {
          C[{i,j}] += ALPHA * A[{i, k}] * B[{k, j}]; // NK*NI*NJ+2*NK*(NI*NJ)
        }
      });
    }));
  }
