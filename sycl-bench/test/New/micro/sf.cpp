...
public:
  ...
  void run(std::vector<cl::sycl::event>& events) {
    // 并行执行特殊函数性能微基准测试的内核逻辑，并将相应的事件存储到 events 向量中
    events.push_back(args.device_queue.submit([&](s::handler& cgh) { // cgh 是处理器对象，表示 SYCL 内核的执行环境
...
int main(int argc, char** argv) { // 这个程序旨在执行一些微基准测试，具体测试了不同数据类型的特殊函数性能。
