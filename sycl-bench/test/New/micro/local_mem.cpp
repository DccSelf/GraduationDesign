  ...
  void run(std::vector<cl::sycl::event>& events) {
    events.push_back(args.device_queue.submit([&](cl::sycl::handler& cgh) {
      ...
      cgh.parallel_for<MicroBenchLocalMemoryKernel<DATA_TYPE, COMP_ITERS>>(ndrange, [=](s::nd_item<1> item) {
        ...
        int lid2 = (item.get_local_id(0) + 1) % item.get_local_range()[0]; // 下一个本地工作项ID

        local_mem[lid] = in[gid]; // 将输入数据从全局复制到局部内存中

        item.barrier(s::access::fence_space::local_space); // 插入本地内存屏障，确保在局部内存的写入和读取之间进行同步
        ...
        // 此内核演示了如何在 SYCL 中使用局部内存，通过 barrier 同步工作组内的工作项，以及在工作组内传递数据。
      });
    })); // submit
  }
