...
template <typename T> class ReductionKernelSingleTask;
...
public:
  ...
  void submit_singletask(std::vector<cl::sycl::event>& events){
    this->submit([this, &events](sycl::buffer<T, 1> *input, sycl::buffer<T, 1> *output,
                        const size_t reduction_size, const size_t num_groups) {
      events.push_back(this->local_reduce_singletask(input, output, reduction_size,
                                      num_groups));
    });
  }
...
private:
  ...
  sycl::event local_reduce_ndrange(sycl::buffer<T,1>* input, sycl::buffer<T,1>* output,
  ...
  sycl::event local_reduce_singletask(sycl::buffer<T,1>* input, sycl::buffer<T,1>* output,
    const size_t reduction_size, const std::size_t num_groups)
  {
    return _args.device_queue.submit([&](sycl::handler &cgh) {
      ...
      cgh.single_task<ReductionKernelSingleTask<T>>([=](){
		  ...
		  for(int group_id=0;group_id<num_groups;group_id++)
		  {
            ...
            {
			  int gid = group_id*group_size + lid;
              scratch[lid] = (gid < reduction_size)?acc[gid]:0;
            }

			for(int k = group_size/2; k > 0; k /= 2) {
                //item.barrier();
...
template<class T>
class ReductionSingleTask : public Reduction<T>
{
public:
  ReductionSingleTask(const BenchmarkArgs &args)
  : Reduction<T>{args}
  {}

  void run(std::vector<cl::sycl::event>& events){
    this->submit_singletask(events);
  }

  static std::string getBenchmarkName() {
    std::stringstream name;
    name << "Pattern_Reduction_SingleTask_";
    name << ReadableTypename<T>::name;
    return name.str();
  }
};
...
int main(int argc, char** argv)
{
  ...
  //app.run< ReductionSingleTask<short>>();
  // app.run< ReductionSingleTask<int>>();
  // app.run< ReductionSingleTask<long long>>();
  app.run< ReductionSingleTask<float>>();
  if(app.deviceSupportsFP64())
    app.run<ReductionSingleTask<double>>();
