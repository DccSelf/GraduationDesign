...
public:
  ...
  void run(std::vector<cl::sycl::event>& events) {
    ...
    events.push_back(args.device_queue.submit(
      ...
      if(Use_ndrange){
        ...
        const int global_size = args.problem_size; 
        cgh.single_task<class ScalarProdKernel<T, Use_ndrange>>([=](){
    ...
    while (array_size!= 1) {
      ...
      events.push_back(args.device_queue.submit(
        [&](cl::sycl::handler& cgh) {
          ...
          if(Use_ndrange) {
			        //Change to SingleTask	
        		cgh.single_task<class ScalarProdReduction<T, Use_ndrange>>([=](){		
			...
			// NDRange parallel_for
            // cgh.parallel_for<class ScalarProdReduction<T, Use_ndrange>>(ndrange,[=](cl::sycl::nd_item<1> item) 
            //   {
            //     size_t gid= item.get_global_linear_id();
  ...
  static std::string getBenchmarkName() {
    ...
    name << "ScalarProduct_";
    name << (Use_ndrange ? "NDRange_To_SingleTask_" : "Hierarchical_");
...
int main(int argc, char** argv)
{
  ...
  if(app.shouldRunNDRangeKernels()) { //change to single_task
