...
public:
  ...
  void submit_singletask(std::vector<cl::sycl::event>& events)
  {
    events.push_back(_args.device_queue.submit([&](sycl::handler& cgh) {
      ...
      const int local_size = _args.local_size;
      const int global_size = _args.problem_size;

      cgh.single_task<ReductionKernelSingleTask<T>>([=]() {

		for(int global_id=0; global_id<global_size;global_id+=local_size)
        {
			int gid = global_id;
...
template<class T>
class SegmentedReductionSingleTask : public SegmentedReduction<T>
{
public:
  SegmentedReductionSingleTask(const BenchmarkArgs &args)
  : SegmentedReduction<T>{args}
  {}
  ...
  static std::string getBenchmarkName() {
    std::stringstream name;
    name << "Pattern_SegmentedReduction_SingleTask_";
    name << ReadableTypename<T>::name;
    return name.str();
  }
};
...
int main(int argc, char** argv)
{
  ...
  // app.run< SegmentedReductionSingleTask<short>>();
  // app.run< SegmentedReductionSingleTask<int>>();
  // app.run< SegmentedReductionSingleTask<long long>>();
  app.run< SegmentedReductionSingleTask<float>>();
  if(app.deviceSupportsFP64())
    app.run<SegmentedReductionSingleTask<double>>();
