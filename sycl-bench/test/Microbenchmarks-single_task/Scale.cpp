...
public:
  MicroBenchScale(const BenchmarkArgs& args)
  ...
  void run(std::vector<s::event>& events) {
    events.push_back(args.device_queue.submit([&](cl::sycl::handler& cgh) {
      ...
      // DataT scalar = 3.0;
      // cgh.parallel_for<MicroBenchScaleKernel<DataT, Dims>>(
      //     global_size, [=](s::id<Dims> gid) { out[gid] = scalar * in[gid]; });

      //single_task
      DataT scalar = 3.0;
      cgh.single_task<MicroBenchScaleKernel<DataT,Dims>>([=,global_size=buffer_size]{
