//clang++ -fsycl-device-only -Xclang -fsycl-int-header=DRAM-single_task-header.h -Xclang -fsycl-int-footer=DRAM-single_task-footer.h -I/thfs1/home/penglin/dcc/sycl-bench/include -O3  DRAM-single_task.cpp -o DRAM-single_task-device.ll
//
#include "common.h"
...
public:
  ...
  void run(std::vector<s::event>& events) {
    events.push_back(args.device_queue.submit([&](cl::sycl::handler& cgh) {
      auto in = input_buf.template get_access<s::access::mode::read>(cgh);
      auto out = output_buf.template get_access<s::access::mode::discard_write>(cgh);
      cgh.single_task<MicroBenchDRAMKernel<DataT, Dims>>([=, global_size = buffer_size]() {
